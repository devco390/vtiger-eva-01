		...
		<div class="editViewPageDiv viewContent">
			<div class="col-sm-12 col-xs-12 content-area {if $LEFTPANELHIDE eq '1'} full-width {/if}">
				...
				<input type="hidden" id="selectedModuleName" name="selectedModuleName" value="{$MODULE_LABEL}" />
					...
					{if $TKSNOTWRITABLE eq 'false' || $ZIPNOTENABELD eq 'false'}
					<div>
					<h6><b style="color:red;">{vtranslate('LBL_WARNING', $MODULE)}</b></h6>
					<ol>
					  {if $TKSNOTWRITABLE eq 'false'}
					  <li style="color:red;">{vtranslate('ISNOTWRITABLE', $MODULE)}</li>
					  {/if}
					   {if $ZIPNOTENABELD eq 'false'}
					  <li style="color:red;">{vtranslate('ZIPNOTENABLE', $MODULE)}</li>
					  {/if}
					</ol> 
					</div>
					{/if}
					<div class='modal-overlay-footer clearfix'>
						<div class="row clearfix">
							<div class='textAlignCenter col-lg-12 col-md-12 col-sm-12 '>
								 {if $TKSNOTWRITABLE eq 'false' || $ZIPNOTENABELD eq 'false'}
								<button type='button' style="visibility:hidden;" id='nextStep' class='btn btn-success saveButton'>{vtranslate('LBL_NEXT', $MODULE)}</button>&nbsp;&nbsp;
								{else}
